...
public class ClassifiedCustomers2 {
    Customers[] classifiedArr = new Customers[GroupType.values().length ]; //3 일반적인걸로 바꿔라

    public ClassifiedCustomers2() {
        for (int i = 0; i < GroupType.values().length; i++) {
            classifiedArr[i] = new Customers();
        }
    }

    public void showClassifiedCustomers(){

        for (int i = 0; i < classifiedArr.length; i++) {
            if (classifiedArr[i] != null) {
                System.out.println(groupTypes[i]);
                Customer[] newArr = Arrays.copyOf(classifiedArr[i].customers,classifiedArr[i].getSize());
    ...
    public void sortedByName(OrderType orderType) {
        ...
        for (int i = 0; i < classifiedArr.length; i++) {
            ...
            if (classifiedArr[i] != null) {
                ...
                System.out.print(groupTypes[i]+" Group : ");
                System.out.println(classifiedArr[i].getSize()+" customers");
                Customer[] newArr = Arrays.copyOf(classifiedArr[i].customers,classifiedArr[i].getSize());
